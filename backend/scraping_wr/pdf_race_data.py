...
from utils_pdf import (handle_table_partitions, get_data_loc, print_stats,
                       clean_df, get_string_loc, clean_convert_to_list, reset_axis_0, clean_str)
...
def df_to_json(df: pd.DataFrame) -> dict:
    ...
    #data = re.findall("\d+\.\d+", str(data_df.values.flatten().tolist()))
...
competition_ids = get_competition_ids(years=2021)
pdf_urls = get_pdf_urls(comp_ids=competition_ids,
                        comp_limit=NO_OF_COMPETITIONS, results=0)[:10]
